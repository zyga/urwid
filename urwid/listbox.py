...
class SimpleListWalker(MonitoredList, ListWalker):
    def __init__(self, contents):
        ...
        """
        if not getattr(contents, '__getitem__', None):
            raise ListWalkerError("SimpleListWalker expecting list like object, got: %r" % (
                contents,))
...
class ListBox(Widget, WidgetContainerMixin):
    ...
    def render(self, size, focus=False):
        ...
        if rows > maxrow:
            raise ListBoxError("Listbox contents too long!  Probably urwid's fault (please report): %r" % (
                (top, middle, bottom),))
        ...
        if rows < maxrow:
            ...
            if trim_bottom != 0 or self.body.get_next(bottom_pos) != (None, None):
                raise ListBoxError("Listbox contents too short!  Probably urwid's fault (please report): %r" % (
                    (top, middle, bottom),))
    ...
    def shift_focus(self, size, offset_inset):
        ...
        if offset_inset >= 0:
            if offset_inset >= maxrow:
                raise ListBoxError("Invalid offset_inset: %r, only %r rows in list box" % (
                    offset_inset, maxrow))
            ...
            if offset_inset + tgt_rows <= 0:
                raise ListBoxError("Invalid offset_inset: %r, only %r rows in target!" % (
    ...
    def change_focus(self, size, position,
        ...
        if offset_inset >= 0:
            ...
            if offset_inset + tgt_rows <= 0:
                raise ListBoxError("Invalid offset_inset: %s, only %s rows in target!" % (
                    offset_inset, tgt_rows))
            self.offset_rows = 0
            self.inset_fraction = (-offset_inset, tgt_rows)
        ...
        if len(cursor_coords) == 1:
            ...
            if pref_row < 0 or pref_row >= tgt_rows:
                raise ListBoxError("cursor_coords row outside valid range for target. pref_row:%r target_rows:%r" % (
                    pref_row, tgt_rows))
    ...
    def get_focus_offset_inset(self, size):
        ...
        if offset_rows == 0:
            ...
            if inum < 0 or iden < 0 or inum >= iden:
                raise ListBoxError("Invalid inset_fraction: %r" % (
                    self.inset_fraction,))
