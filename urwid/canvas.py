...
class CompositeCanvas(Canvas):
    ...
    def overlay(self, other, left, top):
        ...
        assert right >= 0, "top canvas of overlay not the size expected!" + \
            repr((other.cols(), left, right, width))
        assert bottom >= 0, "top canvas of overlay not the size expected!" + \
            repr((other.rows(), top, bottom, height))
